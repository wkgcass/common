...
public interface A1FuncSup<T> extends var {
	...
	@SuppressWarnings("unchecked")
	default <R> R forThose(Predicate<T> predicate, VFunc1<T> func) {
		return (R) forThose(predicate, $(func));
	}

	@SuppressWarnings("unchecked")
	default <R> R forThose(Predicate<T> predicate, VFunc2<T, IteratorInfo<R>> func) {
		return (R) forThose(predicate, $(func));
	}

	default <R> R forThose(Predicate<T> predicate, RFunc1<R, T> func) {
		return forThose(predicate, $(func));
	}

	default <R> R forThose(Predicate<T> predicate, RFunc2<R, T, IteratorInfo<R>> func) {
		return forThose(predicate, $(func));
	}

	<R> R forThose(Predicate<T> predicate, def<R> func);
